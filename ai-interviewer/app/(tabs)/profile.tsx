...
const defaultSkills = ["Python", "FastAPI", "React", "SQL", "AWS"];
...
export default function ProfileScreen() {
  ...
  const { interviews, completedInterviews } = useInterviews();
  ...
  const completedCount = completedInterviews.length || 12;
  const averageScore = interviews.length > 0
    ? Math.round(
        interviews
          .filter(i => i.interview_scores?.[0]?.overall_score)
          .reduce((acc, i) => acc + (i.interview_scores?.[0]?.overall_score || 0), 0) /
        interviews.filter(i => i.interview_scores?.[0]?.overall_score).length || 0
      )
    : 78;
              ...
              <View style={styles.nameContainer}>
                ...
                <Text style={styles.jobTitle}>
                  {user?.role === 'candidate' ? 'Candidate' :
                   user?.role === 'recruiter' ? 'Recruiter' : 'Backend Developer'}
                </Text>
              </View>
            </View>
